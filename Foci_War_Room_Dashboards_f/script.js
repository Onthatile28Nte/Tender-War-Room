...
async function addBid() {
  ...
  if (error) { console.error(error); alert('Failed to save bid: ' + error.message); return; }
  clearBidForm();
  await loadBids();
}
...
function renderBids(){
  ...
  if(!bids.length){
    ...
    body.innerHTML=bids.map((b,i)=>`<tr>
      <td style="font-weight:500">${b.name}</td>
      <td>${b.client}</td>
      <td>${b.value?'R '+b.value.toLocaleString():'—'}</td>
      <td>${b.deadline}</td>
...
function updateMonKPIs(){
  const go=bids.filter(b=>b.score>=60);
  const nogo=bids.filter(b=>b.score>0&&b.score<60);
  const sub=bids.filter(b=>b.status==='Submitted'||b.status==='Awarded').length;
  document.getElementById('m-pipeline').textContent=bids.length;
  document.getElementById('m-submitted').textContent=sub;
  document.getElementById('m-go').textContent=go.length;
  document.getElementById('m-nogo').textContent=nogo.length;
  document.getElementById('m-pipeval').textContent=go.length?'R '+Math.round(go.reduce((s,b)=>s+b.value,0)/1000)+'k':'R 0';
  document.getElementById('bid-count').textContent=bids.length+' bid'+(bids.length!==1?'s':'');
}

function renderGNG(){
  const low=bids.filter(b=>b.score>0&&b.score<60);
  const el=document.getElementById('gng-list');
